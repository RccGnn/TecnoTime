...
<link rel="icon" type="image/svg+xml"
	href="${pageContext.request.contextPath}/images/TecnoTimeIcon.svg">
...
<link rel="stylesheet"
	href="<%= request.getContextPath() %>/styles/offerte.css">
<link rel="stylesheet"
...
<body>
		...
		<div class="productsingle-gallery">
			<div class="productsingle-main-image-wrapper">
				<img id="productsingle-main-image"
					src="${articolo.immagini[0].url != null ? articolo.immagini[0].url : "
					/images/alt-prodotti.png"}" alt="${articolo.nome}">
			</div>
			<div class="productsingle-thumbs">
				<c:forEach var="imgBean" items="${articolo.immagini}">
					<img class="productsingle-thumb" src="${imgBean.url}"
						alt="Foto aggiuntiva">
					<!--  aggiugere la sorgente delle foto o eventuali foto aggiuntive dal DB -->
				</c:forEach>
			</div>
		</div>

		<c:choose>
			<c:when test="${requestScope.messaggio != null}">
				<div class="success-message">"${requestScope.messaggio}"</div>
			</c:when>
		</c:choose>
		<div class="productsingle-details">
			<h1 class="productsingle-title">${articolo.nome}</h1>

			<form id="modifyProduct" method="post" action="AdminModificaProdotto">
				<c:choose>
					<c:when test="${articolo.pdFisico != null}">
						<label for="prezzo">Prezzo del'articolo: </label>
						<input name="price" type="number" step="0.01"
							class="productsingle-price" value="${articolo.pdFisico.prezzo}">

						<label for="descrizione">Prezzo del prodotto: </label>
						<textarea name="description" class="productsingle-description">
					...
					<c:when test="${articolo.pdDigitale != null}">
						<label for="prezzo">Prezzo del'articolo: </label>
						<input name="price" type="number" step="0.01"
							class="productsingle-price" value="${articolo.pdDigitale.prezzo}">
					...
					<c:when test="${articolo.servizio != null}">
						<label for="prezzo">Prezzo del'articolo: </label>
						<input name="price" type="number" step="0.01"
							class="productsingle-price" value="${articolo.servizio.prezzo}">
				...
				<c:choose>
					<c:when test="${articolo.pdFisico != null}">
						<input type="number" id="productsingle-quantity" name="quantity"
							value="1" min="0">
					...
					<c:when test="${articolo.servizio != null}">
						<output id="productsingle-quantity">
							<i><b>Unlimited (Tecnotime own services)</b></i>
						</output>
						<br>
						<!-- aggiungere eventuale controllo per vedere la quantità disponibile del prodotto considerato -->
					</c:when>
	...
	<section class="productsingle-reviews"></section>

	<jsp:include page="footer-amministratore.jsp" />
	<script src="<%= request.getContextPath() %>/js/articolo-single.js"
		defer></script>
</body>
</html>